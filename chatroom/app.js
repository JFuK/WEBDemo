...
var getUserById = function(name, callback) {
	//执行SQL语句
	var sql = 'select * from chatuser where name=?';
	var params = [name];
...
var addUser = function(params, callback) {
	//执行SQL语句
	var sql = 'insert into chatuser(`name`,`pwd`,`email`,`phone`,`create_time`) values(?,?,?,?,?)';
	conn.query(sql, params, function(err, result) {
		if (err) {
...
var updateLoginInfo = function(params) {
	var sql = 'update chatuser set login_time=?,login_ip=? where id=?';
	conn.query(sql, params, function(err, result) {
		if (err) {
